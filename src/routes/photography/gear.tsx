...
const photographyGear: Record<string, { name: string; publicId: string }[]> = {
  ...
  Lens: [
    ...
    { name: "Sigma Art 18-35mm f/1.8 DC HSM", publicId: "SigmaArt18-35mmf1.8" },
    {
      ...
      publicId: "Tamron18-400f3.5-6.3",
    },
    { name: "Sigma 24-70mm f/2.8 EX DG HSM", publicId: "Sigma24-70f2.8" },
    {
      name: "Nikon AF-P DX 70-300mm f/4.5-6.3G ED VR",
      publicId: "Nikon70-300f4.5-6.3g",
    },
    ...
    <PhotographyLayout>
      <div className="container flex flex-col gap-8 pb-12">
        {Object.entries(photographyGear).map(([section, gears]) => (
          <div key={section}>
            <h4 className="mb-4 font-bold text-xl">{section}</h4>
            <ul className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {gears.map((gear) => {
                const url = buildCldUrl(
                  `kdphotography/portfolio/static/portfolio/media-private/gear/${gear.publicId}`,
                );
                return (
                  <li
                    key={gear.publicId}
                    className="flex h-full flex-col items-center justify-center rounded-md border border-ctp-subtext0 border-solid"
                  >
                    <img
                      src={url}
                      alt={gear.name}
