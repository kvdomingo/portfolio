...
<span id="eq-xyz2lab" class="mx-auto">
  $ \begin{equation}
  \begin{aligned}L &= 116 f\left(\frac{Y}
  {Y_n}\right) - 16 \\ a^\star &= 500\left[f\left(\frac{X}
  {X_n}\right) - f\left(\frac{Y}
  {Y_n}\right)\right] \\ b^\star &= 200\left[f\left(\frac{Y}
  {Y_n}\right) - f\left(\frac{Z}
  {Z_n}\right)\right] \end{aligned}
  \end{equation}$
</span>
...
<table class="table" id="tab-color-diff">
  <caption>
    Table 1: Color difference values for the Macbeth patches under D65 and D55
    illumination.
  </caption>
  <thead align="center">
    <tr>
...
<ol className="reference">
  <li id="Soriano2019">
    M. N. Soriano, _Uniform color spaces, color difference, color appearance
    phenomena_ (2019).
  </li>
  <li id="CIE64">
