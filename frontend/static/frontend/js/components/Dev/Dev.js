                ...
                {(!this.state.isLoaded)
                    ...
                    : <Container fluid className='my-5'>
                        {this.state.projects.map((project, i) => (
                            <Card key={i} className='card-image card-cover mb-4' style={{ backgroundImage: `url("${project.cover_photo}")`}}>
                                <div className='text-white text-md-center text-left align-items-center h-100 rgba-black-strong py-5 pt-5 px-4'>
                                    ...
                                    <CardBody>
                                        {(project.organization)
                                            ? <div>
                                                ...
                                                <p className="d-inline">
                                                    <a href={project.organization_url} target='_blank' rel='noopener noreferrer' style={{ color: 'mediumvioletred' }}>
                                                        {project.organization}
                                                    </a>
                                                </p><br />
                                            </div>
                                        ...
                                        <p className="d-inline">{dateFormat(project.start_date, 'mmm yyyy')} &ndash; {(project.end_date)
                                            ? dateFormat(project.end_date, 'mmm yyyy')
                                            : 'present'
                                        } </p><br />
